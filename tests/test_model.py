...
def sentiment_pipeline():
    ...
    assert model_path.exists(), f"Model does not exists on {model_path}"
...
def test_model_accuracy(sentiment_pipeline, test_dataset):
    ...
    if isinstance(test_dataset["labels"][0], str):
        test_dataset = test_dataset.map(lambda x: {"labels": label2id.get(x["labels"].capitalize(), -1)})
    
    eval = evaluate.evaluator("sentiment-analysis")
    result_acc = eval.compute(
...
def test_model_predictions(sentiment_pipeline, text, expected_label):
    """
    Test the model's prediction for individual example texts.
    """
...
def test_batch_inference_output(sentiment_pipeline):
    """
    Test that batch inference returns one prediction per input and valid labels.
    """
    texts = [
        "I love this!", 
...
def test_model_handles_empty_input(sentiment_pipeline):
    """
    The model should handle empty input gracefully (return empty or neutral result).
    """
    result = sentiment_pipeline("")
    
    assert result is not None, "Model returned None for empty input"
    assert isinstance(result, list), f"Expected list, got {type(result)}"
    assert len(result) == 0 or all("label" in r for r in result), (
        f"Unexpected output format: {result}"
    )

def test_model_handles_long_input(sentiment_pipeline):
    """
    The model should not handle more than 512 token input texts without crashing.
    """
    long_text = "This is a very long text. " * 1000  # Create a long text by repetition
    try:
...
def test_model_handles_special_characters(sentiment_pipeline):
    """
    The model should handle texts with special characters and emojis without crashing or invalid output.
    """
    ...
    assert isinstance(result, list) and len(result) == 1, \
        "Unexpected output format for special character input"

    output = result[0]
    assert "label" in output and "score" in output, \
        "Output missing expected keys ('label', 'score')"
...
def test_model_handles_non_english_input(sentiment_pipeline):
    """
    The model should handle non-English texts without crashing or returning invalid output,
    even if predictions may be less accurate.
    """
